...
function varupdate() {
  ...
  if (flaggrashof) {
    ...
    if (r1 == r3 && r2 == r4) {
      ...
      else theta4dash = deg(2 * Math.atan((-kb - Math.sqrt(det)) / (2 * ka)));
    } else {
...
function draw() {
  ...
  if (flaggrashof) {
    ...
    ctx.restore();
    printcomment(
      "<center>Please go to non-Grashof simulation<br>if you wish to work with the given link combination</center>",
      0
    );
  }
}
